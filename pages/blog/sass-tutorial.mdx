...
If you ever had to write css, you have must heard sass or preprocessor. What is preprocessor ? A CSS preprocessor is a program that lets you generate CSS from the preprocessor's own unique syntax.
There are many CSS preprocessor to choose but most popular is Sass. That's why we are going to talk about Sass.

### What is Sass ?

Before speaking from the Sass, firstly I want to talk about why should I use Sass. CSS on its own can be fun, but stylesheets are getting larger, more complex and harder to maintain. That's why we use preprocessors.
...
Finally, we are ready to write scss instead of css. Now, we can do more practice to improve scss skill. You can check it out sass documentation for see lots of example.
